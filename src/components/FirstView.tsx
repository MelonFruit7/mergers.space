...
const FirstView = () => {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      // this will never happen because like if they existed at some point,
      // they're gonna keep existing y'know
      if (!slider.current || !logo.current) return;
    ...
    return () => {
      ...
      logo.current?.removeEventListener(
        "transitionend",
        handleLogoTransitionEnd
      );
    };
  }, [slider.current, logo.current]);

  return (
    <div className={load ? "" : "fixed z-20"}>
      <div
        ref={slider}
        className="h-[100vh] w-[100vw] relative bg-black duration-[3s] ease-in transition-[height] overflow-hidden flex justify-around items-end"
      >
        {[...Array(spaceships)].map((x, i) => (
          <div
            ...
            style={{
              ...
              backgroundSize: `${widthX / (spaceships * 2)}px ${
                widthX / (spaceships * 2)
              }px`,
            }}
      ...
      <div className="h-[100vh]">
        ...
        <div
          ref={logo}
          className="relative h-full flex justify-center items-center opacity-0 duration-[2s] ease-in transition-opacity"
        >
